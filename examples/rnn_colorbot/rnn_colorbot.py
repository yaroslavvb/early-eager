...
class RNNColorbot(tfe.Network):
  ...
  def __init__(self, rnn_cell_sizes, label_dimension, keep_prob, log_dir=None):
    ...
    Args:
      ...
      keep_prob: (1 - dropout probability); dropout is applied to the outputs of
        each LSTM layer
      log_dir: (optional) the directory in which summaries will be written for
        TensorBoard during training. To view summaries, use:
    ...
    if log_dir is not None:
      self.summary_writer = tfe.SummaryWriter(log_dir)
    else:
...
def train_one_epoch(model, optimizer, train_data, log_interval=None):
  ...
  for (labels, chars, sequence_length) in tfe.Iterator(train_data):
    ...
    if log_interval and batch % log_interval == 0:
      ...
      print("train/batch #{}\tloss: {:.6f}".format(batch, float(batch_loss)))
      if model.summary_writer is not None:
...
def main(_):
  ...
  model = RNNColorbot(
      ...
      log_dir=os.path.join(FLAGS.dir, "logs"))
